...
export class CommonHandler<T extends ICommonApi, R, Q> {
	...
	public confirmAction(str: string, param: string | Function, exec?: Function, isRefresh: boolean = true) {
		ElMessageBox.confirm(str, '警告')
			.then(async (res) => {
				let loading = new MyLoading();
				try {
	...
	public async refresh(apiMethod?: Function, callback?: Function) {
		...
		try {
			if (this.pager) {
				...
				if (this.pager.currentPage == 1) this._loadStatus = this.pager.items.length ? LoadStatus.success : LoadStatus.empty;
			} else if (typeof apiMethod == 'function') {
				await apiMethod();
				this._loadStatus = LoadStatus.success;
			}
			this.pager && arguments.length == 1 && typeof apiMethod == 'function' ? apiMethod() : typeof callback == 'function' && callback();
		} catch (error) {
			if (this.pager) {
