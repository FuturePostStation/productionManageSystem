...
const MyTable = defineComponent({
	...
	render() {
		...
		const header = (scope: any, item: IColItem) => {
			const field = (this.tableData as Pager<any, any, any>).listField?.listShowField?.find((el) => el.fieldCode == scope.column.property);
			return (
				<div class={style.thead}>
					<span style="margin-right: 5px;">{item.label}</span>
					{field && (
						<el-icon size="20" color="#999" title={field.isLock ? '解锁' : '锁定'} onClick={() => this.$emit('lock', field)}>
