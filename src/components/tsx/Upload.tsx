...
export default new (class Upload extends PageBase<IProps, IEvent> {
			...
			<div>
				...
				>
					...
					<span v-slot="footer" class="dialog-footer">
						<el-button onClick={this.close} size='default'>关 闭</el-button>
					</span>
				</el-dialog>
			</div>
		);
	...
	private body() {
		...
		return (
			...
			>
				{this.modelValue ? this.content() : <el-icon class={style.icon}><Plus /></el-icon>}
	...
	private content() {
		...
		if (this.localMime.includes('pdf')) {
			child = <img class={style.mediaItem} src="/static/images/pdf.jpg" alt="" onClick={() => window.open(this.modelValue)} />;
		...
		return (
			<div onClick={withModifiers(() => { }, ['stop'])}>
				{!this.disabled && <i class={['el-icon-error', style.close]} onClick={withModifiers(this.onClear, ['stop'])}></i>}
				{child}
			</div>
		);
	}
	...
	private handleSuccess(res: any, file: UploadFile, fileList: UploadFiles) {
		let isWait = fileList.some((item) => item.status != 'success' && item.status != 'fail');
		if (!isWait) {
			if (res?.success) {
				let mime = this.getMime(file.raw!);
				if (fileList.length == 1) {
					this.localMime = mime;
					this.$emit('update:modelValue', res.message);
					if (this.postHandler) this.postHandler(mime);
				}
				let successList: Array<UploadMedia> = [];
