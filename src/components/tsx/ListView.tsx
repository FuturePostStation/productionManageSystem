...
export default new (class ListView<Add, Res extends { id: number }, Query extends IPageSort = IPageSort> extends PageBase<IProps<Query>, IEvent, ISlots> {
	constructor() {
		super();
	}
	...
	private searchNode() {
		let node = this.$slots.searchItems?.();
		if (node || (Array.isArray(node) && (node as AnyArray).length > 0)) {
			return <SearchWidget v-slots={{ default: () => node, btns: this.$slots.searchBtns }} onSearch={() => this.refresh()} onReset={() => this.reset()} />;
		}
	}
	...
	private buildCol() {
		...
		if (this.tableConfig?.setColumns) {
			...
			this.pager.listField.listShowField?.forEach((item) => {
				this.columns[item.fieldCode] = { label: item.fieldName, showOverflowTooltip: true, width: item.width?.toString(), fixed: item.isLock ? 'left' : undefined };
			});
		}
	}
	...
	private addOrEdit(item?: Res) {
		if (this.dialogConfig) {
			...
			if (this.dialogConfig.editConfig) {
				config = { ...this.dialogConfig.editConfig, onConfirm: this.dialogConfig.editConfig.onConfirm || config.onConfirm };
			}
