...
export default new (class SelectDict extends PageBase<IProps, IEvent, ISlots> {
	constructor() {
		super();
	}
	...
	private async getList() {
		if (!this.isGet) {
			...
			try {
				if (this.isInterface) {
					let data = await this.api.dynamicDict(this.dictCode);
					this.list = data.map((item: any) => ({
						text: item.text,
						value: item.text.toString(),
					}));
				} else {
					let data = await this.api.getDict(this.dictCode);
